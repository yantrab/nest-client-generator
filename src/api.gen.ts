...
export const startGenerateClientApi = (config = defualtConfig) => {
    ...
    const models = new Project()
        .addExistingSourceFiles(modelsPath + "/**/*.ts")
        .reduce((names, file) => names.concat(file.getClasses().map(c => c.getName())), [])
    ...
    files.forEach(file => {
        ...
        methods.forEach(method => {
            ...
            replacment = replacment.replace('{resolve}', models.includes(type)? `resolve(plainToClass(${type},<${type}>data))` : 'resolve(data)')
